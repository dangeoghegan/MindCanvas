...
import type { Note, ContentBlock, ContentBlockType, ChatMessage, ChecklistItem, ChatMessageSourceNote, View } from './types';
...
type PrevView = Exclude<View, 'chat' | 'note'>;
const isPrevView = (v: View): v is PrevView => v !== 'chat' && v !== 'note';
...
function App() {
  ...
  // ✅ Use the central View type (includes 'note')
  const [currentView, setCurrentView] = useState<View>('dashboard');

  // ✅ previousView is only for dashboard/library/media
  const [previousView, setPreviousView] = useState<PrevView>('dashboard');
  ...
  // Bottom bar shouldn’t navigate directly to 'note'
  const handleSetView = (view: Exclude<View, 'note'>) => {
    setCurrentView(view);
    setActiveNoteId(null);
  };
